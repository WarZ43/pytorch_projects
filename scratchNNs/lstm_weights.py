...
class LSTMWeights:
    """
    from scratch LSTM network uses SGD and numpy matrixes for better compute speed. parameter N to control how many cells
    fixed to input of size 1, bptt used to train with MSE loss
    """
    ...
    #evaluates the current weights on data, caches intermediate values, and returns MSE loss when shouldMSE = True
    ...
    def bptt(self, data, steps, step_size = None):
        ...
        for i in range(steps):
            ...
            #backpropagation for the final convolutional layer
            ...
            for j in range(len(data)-2,-1,-1):
                ...
                #calculate first level derivatives
                da_f = f[j] * (1 - f[j])
                da_i = inp[j] * (1 - inp[j])
                da_c = 1 - c[j]**2
                da_o = o[j] * (1 - o[j])
                
                #calculate chain rule paths          
